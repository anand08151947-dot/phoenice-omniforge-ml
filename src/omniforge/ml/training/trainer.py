"""AutoML trainer for Phase 7 — with expert diagnostics."""
...
def _build_models(task_type: str, strategy: str) -> list[dict]:
    ...
    if task_type in ("classification", "anomaly_detection"):
        return [
            {"id": str(uuid.uuid4()), "model_name": "LightGBM", "library": "lightgbm", "model": _make_lgbm_clf(cw)},
            {"id": str(uuid.uuid4()), "model_name": "XGBoost", "library": "xgboost", "model": _make_xgb_clf(cw)},
            {"id": str(uuid.uuid4()), "model_name": "Random Forest", "library": "sklearn",
             "model": RandomForestClassifier(n_estimators=200, class_weight=cw, random_state=42, n_jobs=-1)},
            {"id": str(uuid.uuid4()), "model_name": "Logistic Regression", "library": "sklearn",
             "model": LogisticRegression(max_iter=1000, class_weight=cw, random_state=42, n_jobs=-1)},
        ]
    else:
        return [
            {"id": str(uuid.uuid4()), "model_name": "LightGBM", "library": "lightgbm", "model": _make_lgbm_reg()},
            {"id": str(uuid.uuid4()), "model_name": "XGBoost", "library": "xgboost", "model": _make_xgb_reg()},
            {"id": str(uuid.uuid4()), "model_name": "Random Forest", "library": "sklearn",
             "model": RandomForestRegressor(n_estimators=200, random_state=42, n_jobs=-1)},
            {"id": str(uuid.uuid4()), "model_name": "Ridge Regression", "library": "sklearn", "model": Ridge(alpha=1.0)},
        ]
...
def _compute_threshold_analysis(y_true: np.ndarray, y_prob_pos: np.ndarray) -> list[dict]:
    results = []
...
def _detect_leakage(X: np.ndarray, y: np.ndarray, feature_cols: list[str], threshold: float = 0.85) -> list[dict]:
    result = []
...
def _generate_sw(result: dict, is_classifier: bool) -> dict:
    """Generate Strengths and Weaknesses for a trained model."""
    strengths: list[str] = []
    weaknesses: list[str] = []
    ...
    if cv_std < 0.02:
        ...
        weaknesses.append(f"Unstable fold scores (σ={cv_std:.1%}) — check for data issues or leakage")
...
def _train_model_cv(model, X: np.ndarray, y: np.ndarray, cv_splitter, is_classifier: bool) -> dict:
    """Manual CV loop — captures per-fold metrics, probabilities, and aggregated predictions."""
    ...
    if is_classifier:
        f1_val = round(float(f1_score(y_true_arr, y_pred_arr, average="weighted", zero_division=0)), 4)
        ...
        if has_proba:
            ...
            try:
                if n_classes == 2:
                    ...
                    auc = round(float(roc_auc_score(y_true_arr, y_prob_arr, multi_class="ovr", average="weighted")), 4)
            except Exception:
                auc = 0.0
    else:
...
def run_training(
    ...
    for cfg in models_cfg:
        ...
        t0 = time.time()
        try:
            diag = _train_model_cv(model, X, y, cv_splitter, is_classifier)
            result: dict[str, Any] = {
                "id": cfg["id"],
            ...
            result = {
                ...
                "threshold_analysis": [], "hyperparams": {}, "n_features": len(feature_cols),
            }
